...
class Robot02(Robot):
  ...
  def move(self):
    ...
    while True:
      ...
      y = self.ry
      mt = -1
      if (not self.board.isWall(x, y + 1)) and (not self.board.isWall(x, y - 1)) and (not self.board.isWall(x, x + 1)):
        if y < self.board.finishY():
          if self.board.canMove(x, y + 1):
            y = y + 1
            break

        if x < self.board.finishX():
          if self.board.canMove(x + 1, y):
            x = x + 1
            break
